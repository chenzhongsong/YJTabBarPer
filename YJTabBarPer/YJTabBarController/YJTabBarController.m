...
- (void)viewDidLoad {
    ...
    // 自定义tabBar，注释掉后就是使用系统自带的tabBar
    [self setUpTabBar];
}
- (void)setUpTabBar{
    // 自定义tabBar
    YJTabBar *tabBar = [[YJTabBar alloc] initWithFrame:self.tabBar.frame];//没有触发YJTabBar的layoutOutSubviews
...
//    UIViewController *home = [[UIViewController alloc] init];
    ...
    //使用系统自带的tabBar时，不需要这句，会奔溃，直接修改由标签栏控制器管理的一个标签栏是不允许的。
...
//    vc.tabBarItem.title = title;
    ...
    //2.
    //或者创建一个数组，把视图控制器放到一个数组里，最后设置self.viewControllers = arr;

...
- (void)setItems:(NSArray *)items{
    ...
    for (UITabBarItem * item in _items) {
        ...
        //下面执行initWithFrame方法 -》触发其layoutOutSubViews 2次 -》下面的代码
...
-(void)btnClick:(UIButton *)button{
    _selectedButton.selected = NO;//第一次加载是nil
    button.selected = YES;
...
- (void)layoutSubviews{
    ...
    for (UIView *tabBarButton in self.buttons) {
        ...
        btnX = i * btnW;
        tabBarButton.frame = CGRectMake(btnX, btnY, btnW, btnH);//触发其layoutOutSubviews
        i++;
    }
